...
#   service — writes to /etc/open-webui/env      (read by systemd unit)
#
...
SERVICE_ENV="/etc/open-webui/env"
...
Usage: $(basename "$0") [--mode local|service]
  ...
  --mode service  Write to ${SERVICE_ENV} (requires root)
...
if [[ "$MODE" == "service" && "$EUID" -ne 0 ]]; then
    echo "Service mode requires root — re-running with sudo..."
    exec sudo bash "$0" --mode service
fi
...
[[ "$MODE" == "service" ]] && mkdir -p "$(dirname "$ENV_FILE")"
...
if [[ "$MODE" == "local" ]]; then
    ...
    echo "Run: sudo systemctl restart open-webui"
fi
