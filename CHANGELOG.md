...
- 🔒 **Generated uv.lock file.** Created comprehensive lock file with 1,087 pinned transitive dependencies for reproducible builds.
- 🐳 **Docker base image update.** Changed from `python:3.11.14-slim-bookworm` to `python:3.14-slim-bookworm`.

### Added

- ✨ **Multiple requirements files.** Added `requirements_locked.txt` (330 exact pinned versions for production), `requirements_direct.txt` (78 direct dependencies for minimal installs), and `requirements_permissive.txt` (330 packages with `>=` constraints for flexible updates).
- 🔄 **Updated CI/CD workflows.** Release workflow now uses Python 3.14; format-backend tests run on Python 3.13.x and 3.14.x.
