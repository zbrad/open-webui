...
def _require_enabled() -> None:
    if not MODEL_SERVICES_ENABLE:
        raise HTTPException(status_code=503, detail='Model services are disabled')


@router.get('/')
async def list_model_services(user=Depends(get_admin_user)):
    _require_enabled()

    catalog = _load_catalog(MODEL_SERVICES_ALIASES_PATH)
...
def _resolve_entry(name: str) -> dict:
    catalog = _load_catalog(MODEL_SERVICES_ALIASES_PATH)
...
async def start_model_service(name: str, user=Depends(get_admin_user)):
    _require_enabled()
    _resolve_entry(name)  # validate against the catalog before touching a subprocess

    if not os.path.isfile(MODEL_SERVICES_LAUNCHER_PATH):
        raise HTTPException(status_code=500, detail=f'Launcher not found: {MODEL_SERVICES_LAUNCHER_PATH}')

    rc, out, err = await _run(MODEL_SERVICES_LAUNCHER_PATH, '--model', name, 'start')
    if rc != 0:
...
async def stop_model_service(name: str, user=Depends(get_admin_user)):
    _require_enabled()
    _resolve_entry(name)  # validate against the catalog before touching a subprocess
    ...
    rc, out, err = await _run(MODEL_SERVICES_LAUNCHER_PATH, '--model', name, 'stop')
    if rc != 0:
